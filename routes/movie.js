...
async function getRecommended(movieId, page_num) {
    try {
      const response = await fetch("https://api.themoviedb.org/3/movie/"+ movieId + "/recommendations?api_key=" + apiKey + "&language=en-US&page=" + page_num
      );
      const data = await response.json();
      return data;
    } catch (e) {
...
function getCrew(allCrew){
  ...
  for(let crew of allCrew){
    ...
    if(crew.job === "Writer" || crew.job === "Novel" ||crew.job === "Screenplay" ){
      writers.push(crew);
    }
...
router.get('/:movie_id', async (req, res)=>{
    ...
    let page_num = 1;
    try{
        const response = await fetch("https://api.themoviedb.org/3/movie/"+ movieId + "?api_key=" + apiKey + "&append_to_response=videos,credits,reviews,similar,images&include_image_language=en,null&language=en-US");
